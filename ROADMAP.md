...
| P0-06 | Linux Wayland spike (read-only design OK) | `later`   | `plat/p0-06-wayland-spike`                                                                       | P1-01      | Document whether Wayland can support hotkeys/paste/mic with portals; no requirement to ship in Phase 0                                                                                                             |
...
| ID    | Feature                             | Status    | Suggested branch             | Depends on            | Acceptance criteria                                                                                                                                    |
| ----- | ----------------------------------- | --------- | ---------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| P2-01 | Intent router (structured)          | `in progress` | `feat/av-router`          | P1-01                 | Transcript maps to a typed intent; dictation/app/site/agent paths go through one router; tests cover conflicts (“open Claude” app vs website). Plan: [assistant-verb-stack](docs/superpowers/plans/2026-07-26-assistant-verb-stack-plan.md) |
...
| P2-04 | Allowlisted shell commands          | `partial` | `feat/av-pack-project`       | P2-01                 | Project profile (T3.3) + project/job/editor verbs (T3.4) via supervisor; profile miss refuses with checked files. `commands.json` remains for global custom commands. No free-form shell from raw speech. |
...
| P2-06 | Agent runner interface              | `partial` | `feat/av-agent-verbs`        | P2-01, P2-05          | T7.2: `agent.task` seeds last failing job (UI-EDIT-06), shows diff before apply confirm (UI-EDIT-05); brain cannot self-approve R3 force push. CodexRunner kept with BrainProtocol TODO hooks until T7.1. |
...
| P2-08 | History UI                          | `partial` | `feat/av-history-v2`         | —                     | T8.1: history `user_version=2` audit columns (verb/rung/risk/status/slots/evidence/…) with redaction; keep `cleanup_status` route tags one release. UI browse/search still open. |
...
| P2-11 | Visual guide + first-result act     | `partial` | `explore/visual-computer-use` | P2-01, P2-05        | V0–V3 + dogfood repair: live `screen`/`input`/`guide_brain` wired into `browser.result.open`; unknown `app.open` → Google search (no storefront URL hardcodes); confirm surfaces PARTIAL. Win/Linux capture stubs; multi-display + ScreenCaptureKit later (V4). Spec: [visual-guide-act](docs/superpowers/specs/2026-07-27-visual-guide-act-design.md); plan: [openclicky-informed repair](docs/superpowers/plans/2026-07-27-openclicky-informed-assistant-repair-plan.md) |
