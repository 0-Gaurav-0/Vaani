...
```text
vaani/
  core/           # intent parse, route, confirm, history, groq, policy
  adapters/
    linux_x11/    # current implementation extracted
    macos/        # Phase 1
    windows/      # Phase 1
  remote/         # Phase 3
```
...
OSAdapter
  ...
  register_hotkeys(callbacks)
  paste(text) / set_clipboard(text)
  open_app(target) / open_url(url, browser?)
  notify(title, body)
  get_focused_target() -> snapshot | None
```

| ID | Feature | Status | Suggested branch | Depends on | Acceptance criteria |
|---|---|---|---|---|---|
| P1-01 | Define `OSAdapter` protocol + extract Linux | `todo` | `plat/p1-01-os-adapter` | — | Linux path runs through an adapter interface; unit tests use fakes; no behavior regression on Ubuntu/X11 |
| P1-02 | macOS adapter MVP | `todo` | `plat/p1-02-macos-adapter` | P1-01 | Mic capture, at least one global hotkey path, clipboard paste into focused app, open app by bundle/name, notifications; documented permissions (Accessibility, Microphone, Input Monitoring) |
| P1-03 | Windows adapter MVP | `todo` | `plat/p1-03-windows-adapter` | P1-01 | Same MVP as macOS: mic, hotkey, paste, open app/URL, notify; documented Win10/11 assumptions |
| P1-04 | Shared config/paths per OS | `todo` | `plat/p1-04-config-paths` | P1-01 | XDG on Linux, `~/Library/Application Support/Vaani` on macOS, `%APPDATA%\Vaani` on Windows; sites/history/logs land in the right place |
| P1-05 | Cross-platform install docs | `todo` | `docs/p1-05-install-matrix` | P1-02 or P1-03 | README (or `docs/install/`) matrix: OS → deps → permissions → smoke test |
